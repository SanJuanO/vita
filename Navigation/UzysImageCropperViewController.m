...
- (id)initWithImage:(UIImage*)newImage andframeSize:(CGSize)frameSize andcropSize:(CGSize)cropSize
{
	...
	if (self) {
        ...
        UINavigationBar *navigationBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0.0, 0, self.view.frame.size.width, 64.0)];
        [navigationBar setBarStyle:UIBarStyleDefault];
        [navigationBar setTranslucent:NO];
        
        UINavigationItem *aNavigationItem = [[UINavigationItem alloc] initWithTitle:NSLocalizedString(@"Ajusta al recuadro",)];
        ...
        [btn_rotation setFrame:CGRectMake(0 , cropperView.frame.size.height - 40 , 44, 32)];
        [btn_rotation addTarget:self action:@selector(actionRotation:) forControlEvents:UIControlEventTouchUpInside];
        [btn_rotation setImage:[UIImage imageNamed:@"nc_crop_reset_btn.png"] forState:UIControlStateNormal];
        [btn_rotation setImage:[UIImage imageNamed:@"nc_crop_reset_btn_h.png"] forState:UIControlStateHighlighted];
        btn_rotation.layer.shadowColor = [self colorDesdeString:@"#FF0000" withAlpha:1.0].CGColor;
        btn_rotation.layer.shadowOpacity = 1;
        btn_rotation.layer.shadowRadius = 2;
        btn_rotation.layer.shadowOffset = CGSizeMake(2, 2);
        [self.view addSubview:btn_rotation];

        UIButton *btn_restore = [UIButton buttonWithType:UIButtonTypeCustom];
        [btn_restore setFrame:CGRectMake(cropperView.frame.size.width -44 , cropperView.frame.size.height - 40 , 44, 33)];
        [btn_restore addTarget:self action:@selector(actionRestore:) forControlEvents:UIControlEventTouchUpInside];
        [btn_restore setImage:[UIImage imageNamed:@"nc_crop_rotate_btn.png"] forState:UIControlStateNormal];
        [btn_restore setImage:[UIImage imageNamed:@"nc_crop_rotate_btn_h.png"] forState:UIControlStateHighlighted];
        btn_restore.layer.shadowColor = [self colorDesdeString:@"#FF0000" withAlpha:1.0].CGColor;
        btn_restore.layer.shadowOpacity = 1;
        btn_restore.layer.shadowRadius = 2;
        btn_restore.layer.shadowOffset = CGSizeMake(2, 2);
