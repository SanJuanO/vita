...
- (id)initWithImage:(UIImage*)newImage andframeSize:(CGSize)frameSize andcropSize:(CGSize)cropSize
{
    ...
    if(self)
    {
        ...
        //_imageScale = frameSize.width / self.inputImage.size.width; //scale criteria depend on width size
        ...
        _cropperView.layer.borderWidth=1.0;
        
        _imageScale = 320*_imageScale/_imgView.frame.size.width;
        CGRect imgViewBound2 = CGRectMake(0, 0, _inputImage.size.width*_imageScale, _inputImage.size.height*_imageScale);   //이미지가 생성될 사이즈.
        _imgView.frame=imgViewBound2;
        _imgView.center = self.center;
